...
function withTimeout<T>(promise: Promise<T>, timeoutMs: number | undefined): Promise<T> {
  ...
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error(`Snowflake query timed out after ${timeoutMs}ms`)), timeoutMs)
...
export async function querySnowflake(query: string, options: QueryOptions = {}): Promise<Record<string, any>[]> {
  const { callersRights = false, binds, timeoutMs } = options
  ...
  if (serviceToken) {
    if (callersRights) {
      ...
      return withTimeout(queryWithPool(getCallersPool(combinedToken, serviceToken), query, "spcs-caller", binds), timeoutMs)
    }
    // Retry once on terminated connection (service token rotated mid-flight).
    try {
      return await withTimeout(queryWithPool(getOwnersPool(serviceToken), query, "spcs-owner", binds), timeoutMs)
    } catch (err) {
      ...
      if (msg.includes("terminated connection")) {
        ...
        ownersPool?.drain()
        ownersPool = null
        return withTimeout(queryWithPool(getOwnersPool(serviceToken), query, "spcs-owner", binds), timeoutMs)
      }
      throw err
    }
  }
  ...
  if (process.env.SNOWFLAKE_USER && process.env.SNOWFLAKE_PASSWORD) {
    return withTimeout(queryWithPool(getPasswordPool(), query, "password", binds), timeoutMs)
  }

  // ~/.snowflake/connections.toml or config.toml: use the default connection (local dev)
  const tomlConn = readTomlDefaultConnection()
  if (tomlConn) {
    return withTimeout(queryWithPool(getTomlPool(tomlConn), query, "toml", binds), timeoutMs)
  }
